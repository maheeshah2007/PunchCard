...
export const Auth = {
  google: (credential: string) =>
    request<{ ok: boolean; token: string; user: GoogleUser }>("/auth/google", {
      method: "POST",
...
export const Users = {
  ...
  setRole: (role: string, headers: Record<string, string>) =>
    request<{ ok: boolean; role: string }>("/users/me/role", { method: "PUT", body: JSON.stringify({ role }), headers }),
};
...
export const Templates = {
  create: (data: { name: string; total_stamps: number; reward_description: string; style: string }, headers: Record<string, string>) =>
...
export const UserCards = {
  ...
  redeemReward: (punchcardId: number, headers: Record<string, string>) => request<UserPunchCard>(`/user/punchcards/${punchcardId}/redeem`, { method: "POST", headers }),
};

export const AuthCodes = {
  generate: (templateId: number, headers: Record<string, string>) =>
    request<AuthCodeResult>("/auth-codes/generate", { method: "POST", body: JSON.stringify({ template_id: templateId }), headers }),
  redeem: (code: string, businessId: number, headers: Record<string, string>) =>
    request<{ ok: boolean; stamps_collected: number; is_completed: boolean; punchcard: UserPunchCard }>("/auth-codes/redeem", { method: "POST", body: JSON.stringify({ code, business_id: businessId }), headers }),
};
